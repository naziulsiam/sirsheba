        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              {t('amount')}: {formatTaka(totalPending, lang)}
            </p>
          </div>
        </div>
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground">
                  {student.batch?.name || (lang === 'bn' ? 'ব্যাচ নেই' : 'No Batch')}
                </p>
              </div>
              <div className="text-right">
                <p className="text-sm font-semibold text-destructive">
                  {formatTaka(student.pending, lang)}
                </p>
                {student.daysSincePayment > 30 && (
                  <span className="inline-flex items-center rounded bg-destructive/10 px-1.5 py-0.5 text-[10px] font-medium text-destructive">
                    {formatNumber(student.daysSincePayment, lang)} {lang === 'bn' ? 'দিন' : 'days'}
                  </span>
                )}
              </div>
              <button
